...
namespace OtoServisSatis.WebUI.Controllers
{
    public class AracController : Controller
    {
        ...
        }public async Task<IActionResult> Ara(string q)
        {
            ...
            var model = await _serviceArac.GetCustomCarList(c=> c.SatistaMi && c.Marka.Adi.Contains(q.Trim()) 
            || c.KasaTipi.Contains(q.Trim()) || c.Modeli.Contains(q.Trim()) || c.ModelYili.ToString().Contains(q.Trim()));
                return View(model);
        }
    }
}
